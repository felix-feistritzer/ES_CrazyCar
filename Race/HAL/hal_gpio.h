...
#define LCD_SPI_CS_LOW (P8OUT &= ~LCD_SPI_CS)
#define LCD_SPI_CS_HIGH (P8OUT |= LCD_SPI_CS)
#define LCD_RESET_LOW (P9OUT &= ~LCD_RESET)
#define LCD_RESET_HIGH (P9OUT |= LCD_RESET)

// ****************************************

#define PWM_60HZ            0xA2C2  // 2,5MHz / 60Hz => 41666,67

// ****************************************

typedef struct {
    uint8_t active;     // TRUE 1 / FALSE 0
    uint8_t button;     // START_BUTTON 0 / STOP_BUTTON 1
}ButtonCom;
